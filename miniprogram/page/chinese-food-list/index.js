...
Page({
  ...
  data: {
    ...
    // 页面分类（用于API过滤）
    pageCategory: '寻味中国' // 固定分类，用于统一API请求
  },
  ...
  // 从 API 获取寻味中国数据（统一使用blogApi.blogPostApi.getList）
  fetchMenuLinks(isLoadMore = false, isPreload = false) {
    ...
    // 获取过滤条件
    // 固定使用页面分类（寻味中国），同时支持子分类和搜索关键词
    const pageCategory = this.data.pageCategory || '寻味中国'
    ...
    console.log('[fetchMenuLinks] 请求参数:', {
      page: requestPage,
      pageSize: pageSize,
    ...
    return blogApi.blogPostApi.getList({
      ...
      console.log('[fetchMenuLinks] API响应:', result)
      ...
      if (result && result.success && result.data && Array.isArray(result.data)) {
        ...
        console.warn('[fetchMenuLinks] 未识别的响应格式:', result)
        items = []
        hasMore = false
      }
      ...
      items = items.map(item => {
        ...
        if (item.custom_fields) {
          try {
            ...
            console.warn('[fetchMenuLinks] 解析custom_fields失败:', e)
          }
        }
      ...
      console.error('[fetchMenuLinks] 失败:', error)
