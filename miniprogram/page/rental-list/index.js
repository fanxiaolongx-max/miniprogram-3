...
Page({
  ...
  data: {
    ...
    // 页面分类（用于API过滤）
    pageCategory: '租房酒店' // 固定分类，用于统一API请求
  },
  ...
  // 从 API 获取租房数据（统一使用blogApi.blogPostApi.getList）
  fetchRentals(isLoadMore = false, isPreload = false) {
    ...
    // 获取过滤条件
    // 固定使用页面分类（租房酒店），同时支持子分类和搜索关键词
    const pageCategory = this.data.pageCategory || '租房酒店'
    ...
    return blogApi.blogPostApi.getList({
      ...
      console.log('[fetchRentals] API响应:', result)
      ...
      if (result && result.success && result.data && Array.isArray(result.data)) {
        ...
        console.warn('[fetchRentals] 未识别的响应格式:', result)
        items = []
        hasMore = false
      }
      ...
      items = items.map(item => {
        ...
        if (item.custom_fields) {
          try {
            ...
            console.warn('[fetchRentals] 解析custom_fields失败:', e)
          }
        }
      ...
      console.error('[fetchRentals] 失败:', error)
