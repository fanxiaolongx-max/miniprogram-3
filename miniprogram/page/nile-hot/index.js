...
Page({
  ...
  fetchNileHot(isLoadMore = false, isPreload = false) {
    // 如果是加载更多，设置 loadingMore；否则设置 loading
    if (isLoadMore) {
      if (isPreload) {
        // 预加载时不显示 loadingMore，避免影响用户体验
        console.log('[fetchNileHot] 预加载模式，不显示加载提示')
      } else {
        this.setData({
          loadingMore: true
        })
    ...
    blogApi.blogPostApi.getList({
      ...
      console.log('[fetchNileHot] API响应:', result)
      ...
      if (result && result.success && result.data && Array.isArray(result.data)) {
        ...
        console.error('获取尼罗河热映失败：返回格式不正确', result)
        if (isLoadMore) {
      ...
      if (!Array.isArray(items)) {
        ...
        const newItems = items.map((item, index) => {
          ...
          if (existingIds.has(uniqueId)) {
            console.warn(`[fetchNileHot] 发现重复id: ${uniqueId}，生成新id`)
            uniqueId = `item_${requestPage}_${index}_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`
          }
          ...
          return {
            ...
            title: item.title || item.name || '未知应用',
        ...
        const uniqueNewItems = isLoadMore 
          ? newItems.filter(item => {
              ...
              if (exists) {
                console.warn(`[fetchNileHot] 过滤重复项: id=${item.id}`)
              }
              return !exists
            })
          : newItems
        ...
        allItems.forEach(item => {
          ...
          if (!uniqueItemsMap.has(itemId)) {
            ...
            console.warn(`[fetchNileHot] 最终去重：发现重复id: ${itemId}，已跳过`)
          }
        })
        const finalItems = Array.from(uniqueItemsMap.values())
        
        console.log(`[fetchNileHot] 数据处理：API返回 ${items.length} 条，处理后 ${newItems.length} 条，过滤重复后 ${uniqueNewItems.length} 条，合并后 ${allItems.length} 条，最终去重后 ${finalItems.length} 条`)
        ...
        if (isPreload) {
          ...
          console.log(`[fetchNileHot] 预加载完成，请求页: ${requestPage}，更新后页码: ${nextPage}，hasMore: ${hasMore}，items数量: ${finalItems.length}（新增 ${newItems.length} 条）`)
        } else if (isLoadMore) {
          // 正常加载更多完成
          updateData.loadingMore = false
          updateData.preloadTriggered = false // 重置预加载标记，允许下次预加载
          console.log(`[fetchNileHot] 加载更多完成，请求页: ${requestPage}，更新后页码: ${nextPage}，hasMore: ${hasMore}，items数量: ${finalItems.length}（新增 ${newItems.length} 条，原有 ${existingItems.length} 条）`)
        } else {
          // 首次加载完成
          updateData.preloadTriggered = false // 重置预加载标记
          console.log(`[fetchNileHot] 首次加载完成，请求页: ${requestPage}，更新后页码: ${nextPage}，hasMore: ${hasMore}，items数量: ${finalItems.length}`)
        }
        ...
        if (isLoadMore) {
          if (isPreload) {
            ...
            console.error('[fetchNileHot] 预加载失败，已重置预加载状态')
          } else {
            this.setData({ loadingMore: false })
            wx.showToast({
              title: '加载失败，请重试',
  ...
  loadMore() {
    if (this.data.hasMore && !this.data.loadingMore && !this.data.preloading && !this.data.loading) {
      ...
      wx.createSelectorQuery().in(this).selectViewport().scrollOffset((res) => {
        ...
        // 调用加载函数
        this.fetchNileHot(true, false) // 第二个参数 false 表示不是预加载
      }).exec()
    }
  },
  ...
  onPullDownRefresh() {
    ...
    // 注意：fetchNileHot 内部会设置 loading: false，这里延迟停止下拉刷新
    setTimeout(() => {
