Page({
  ...
  fetchTipGuide(isLoadMore = false, isPreload = false) {
    ...
    if (isLoadMore) {
      if (isPreload) {
        ...
        console.log('[fetchTipGuide] 预加载模式，不显示加载提示')
      } else {
        this.setData({
          loadingMore: true
        })
      ...
      success: (res) => {
        ...
        const newItems = items.map((item, index) => {
          ...
          if (existingIds.has(uniqueId)) {
            console.warn(`[fetchTipGuide] 发现重复id: ${uniqueId}，生成新id`)
            uniqueId = `item_${requestPage}_${index}_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`
          }
          ...
          if (item.custom_fields) {
            try {
              ...
              console.warn('[fetchTipGuide] 解析custom_fields失败:', e)
            }
          }
        ...
        const uniqueNewItems = isLoadMore 
          ? newItems.filter(item => {
              ...
              if (exists) {
                console.warn(`[fetchTipGuide] 过滤重复项: id=${item.id}`)
              }
              return !exists
            })
          : newItems
        ...
        allItems.forEach(item => {
          ...
          if (!uniqueItemsMap.has(itemId)) {
            ...
            console.warn(`[fetchTipGuide] 最终去重：发现重复id: ${itemId}，已跳过`)
          }
        })
        const finalItems = Array.from(uniqueItemsMap.values())
        
        console.log(`[fetchTipGuide] 数据处理：API返回 ${items.length} 条，处理后 ${newItems.length} 条，过滤重复后 ${uniqueNewItems.length} 条，合并后 ${allItems.length} 条，最终去重后 ${finalItems.length} 条`)
        ...
        console.log(`[fetchTipGuide] 准备更新数据，请求页: ${requestPage}，更新后页码: ${nextPage}，isLoadMore: ${isLoadMore}`)
        ...
        if (isPreload) {
          ...
          console.log(`[fetchTipGuide] 预加载完成，请求页: ${requestPage}，更新后页码: ${nextPage}，hasMore: ${hasMore}，items数量: ${finalItems.length}（新增 ${newItems.length} 条）`)
        } else if (isLoadMore) {
          // 正常加载更多完成
          updateData.loadingMore = false
          updateData.preloadTriggered = false // 重置预加载标记，允许下次预加载
          console.log(`[fetchTipGuide] 加载更多完成，请求页: ${requestPage}，更新后页码: ${nextPage}，hasMore: ${hasMore}，items数量: ${finalItems.length}（新增 ${newItems.length} 条，原有 ${existingItems.length} 条）`)
        } else {
          // 首次加载完成
          updateData.preloadTriggered = false // 重置预加载标记
          console.log(`[fetchTipGuide] 首次加载完成，请求页: ${requestPage}，更新后页码: ${nextPage}，hasMore: ${hasMore}，items数量: ${finalItems.length}`)
        }
        ...
        this.setData(updateData, () => {
          ...
          if (isLoadMore && !isPreload && this._savedScrollTop !== undefined && this._savedScrollTop > 0) {
            ...
            setTimeout(() => {
              ...
              console.log(`[fetchTipGuide] 恢复滚动位置到: ${this._savedScrollTop}px`)
              // 重置保存的滚动位置
              this._savedScrollTop = 0
            }, 100)
          }
      ...
      fail: (err) => {
        ...
        if (isLoadMore) {
          if (isPreload) {
            ...
            console.error('[fetchTipGuide] 预加载失败，已重置预加载状态')
          } else {
            this.setData({ loadingMore: false })
            wx.showToast({
              title: '加载失败，请重试',
  ...
  loadMore() {
    if (this.data.hasMore && !this.data.loadingMore && !this.data.preloading && !this.data.loading) {
      ...
      wx.createSelectorQuery().in(this).selectViewport().scrollOffset((res) => {
        ...
        // 调用加载函数
        this.fetchTipGuide(true, false) // 第二个参数 false 表示不是预加载
      }).exec()
    }
  },
  ...
  onPullDownRefresh() {
    ...
    this.fetchTipGuide(false)
    // 注意：fetchTipGuide 内部会设置 loading: false，这里延迟停止下拉刷新
    setTimeout(() => {
