Page({
  onShareAppMessage() {
    return {
      title: '二手集市',
      path: 'page/second-hand-list/index'
    }
  },
  ...
  // 从 API 获取二手集市数据
  fetchSecondHand(isLoadMore = false, isPreload = false) {
    ...
    if (isLoadMore) {
      if (isPreload) {
        ...
        console.log('[fetchSecondHand] 预加载模式，不显示加载提示')
      } else {
        this.setData({
          loadingMore: true
        })
    ...
    console.log(`[fetchSecondHand] 请求参数：isLoadMore=${isLoadMore}, currentPage=${currentPage}, requestPage=${requestPage}, pageSize=${pageSize}, category=${category || '无'}, keyword=${keyword || '无'}`)
    ...
    wx.request({
      ...
      success: (res) => {
        ...
        if (res.data.data && Array.isArray(res.data.data)) {
          ...
          console.log(`[fetchSecondHand] 分页数据：请求页 ${requestPage}，返回 ${items.length} 条，总计 ${total}，还有更多：${hasMore}`)
        }
        // 处理数组格式（format=array 时）：[...]
          ...
          console.log(`[fetchSecondHand] 数组格式：请求页 ${requestPage}，返回 ${items.length} 条，还有更多：${hasMore}`)
        }
        // 兼容旧格式：{ items: [...] }
        ...
        if (!Array.isArray(items)) {
          console.error('获取二手集市数据失败：返回格式不正确')
          if (isLoadMore) {
            this.setData({ loadingMore: false })
          } else {
            this.showError()
        ...
        const newItems = items.map((item, index) => {
          ...
          if (existingIds.has(uniqueId)) {
            console.warn(`[fetchSecondHand] 发现重复id: ${uniqueId}，生成新id`)
            uniqueId = `item_${requestPage}_${index}_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`
          }
        ...
        const uniqueNewItems = isLoadMore 
          ? newItems.filter(item => {
              ...
              if (exists) {
                console.warn(`[fetchSecondHand] 过滤重复项: id=${item.id}`)
              }
              return !exists
            })
          : newItems
        ...
        allItems.forEach(item => {
          ...
          if (!uniqueItemsMap.has(itemId)) {
            ...
            console.warn(`[fetchSecondHand] 最终去重：发现重复id: ${itemId}，已跳过`)
          }
        })
        const finalItems = Array.from(uniqueItemsMap.values())
        
        console.log(`[fetchSecondHand] 数据处理：API返回 ${items.length} 条，处理后 ${newItems.length} 条，过滤重复后 ${uniqueNewItems.length} 条，合并后 ${allItems.length} 条，最终去重后 ${finalItems.length} 条`)
        ...
        console.log(`[fetchSecondHand] 准备更新数据，请求页: ${requestPage}，更新后页码: ${nextPage}，isLoadMore: ${isLoadMore}`)
        ...
        if (isPreload) {
          ...
          console.log(`[fetchSecondHand] 预加载完成，请求页: ${requestPage}，更新后页码: ${nextPage}，hasMore: ${hasMore}，items数量: ${finalItems.length}（新增 ${newItems.length} 条）`)
        } else if (isLoadMore) {
          // 正常加载更多完成
          updateData.loadingMore = false
          updateData.preloadTriggered = false // 重置预加载标记，允许下次预加载
          console.log(`[fetchSecondHand] 加载更多完成，请求页: ${requestPage}，更新后页码: ${nextPage}，hasMore: ${hasMore}，items数量: ${finalItems.length}（新增 ${newItems.length} 条，原有 ${existingItems.length} 条）`)
        } else {
          // 首次加载完成
          updateData.preloadTriggered = false // 重置预加载标记
          console.log(`[fetchSecondHand] 首次加载完成，请求页: ${requestPage}，更新后页码: ${nextPage}，hasMore: ${hasMore}，items数量: ${finalItems.length}`)
        }
        ...
        this.setData(updateData, () => {
          ...
          if (isLoadMore && !isPreload && this._savedScrollTop !== undefined && this._savedScrollTop > 0) {
            ...
            setTimeout(() => {
              ...
              console.log(`[fetchSecondHand] 恢复滚动位置到: ${this._savedScrollTop}px`)
              // 重置保存的滚动位置
              this._savedScrollTop = 0
            }, 100)
          }
      ...
      fail: (err) => {
        ...
        if (isLoadMore) {
          if (isPreload) {
            ...
            console.error('[fetchSecondHand] 预加载失败，已重置预加载状态')
          } else {
            this.setData({ loadingMore: false })
            wx.showToast({
              title: '加载失败，请重试',
  ...
  loadMore() {
    if (this.data.hasMore && !this.data.loadingMore && !this.data.preloading && !this.data.loading) {
      ...
      wx.createSelectorQuery().in(this).selectViewport().scrollOffset((res) => {
        ...
        // 调用加载函数
        this.fetchSecondHand(true, false) // 第二个参数 false 表示不是预加载
      }).exec()
    }
  },
  ...
  onPullDownRefresh() {
    ...
    this.fetchSecondHand(false)
    // 注意：fetchSecondHand 内部会设置 loading: false，这里延迟停止下拉刷新
    setTimeout(() => {
