...
Page({
  ...
  data: {
    ...
    // 页面分类（用于API过滤）
    pageCategory: '二手市场' // 固定分类，用于统一API请求
  },
  ...
  // 从 API 获取二手集市数据（统一使用blogApi.blogPostApi.getList）
  fetchSecondHand(isLoadMore = false, isPreload = false) {
    ...
    // 获取过滤条件
    // 固定使用页面分类（二手市场），同时支持子分类和搜索关键词
    const pageCategory = this.data.pageCategory || '二手市场'
    ...
    console.log('[fetchSecondHand] 请求参数:', {
      page: requestPage,
      pageSize: pageSize,
    ...
    return blogApi.blogPostApi.getList({
      ...
      console.log('[fetchSecondHand] API响应:', result)
      ...
      if (result && result.success && result.data && Array.isArray(result.data)) {
        ...
        console.warn('[fetchSecondHand] 未识别的响应格式:', result)
        items = []
        hasMore = false
      }
      ...
      items = items.map(item => {
        ...
        if (item.custom_fields) {
          try {
            ...
            console.warn('[fetchSecondHand] 解析custom_fields失败:', e)
          }
        }
      ...
      console.error('[fetchSecondHand] 失败:', error)
