      ...
      success: (res) => {
        ...
        const newItems = items.map((item, index) => {
          ...
          if (existingIds.has(uniqueId)) {
            console.warn(`[fetchHotSpots] 发现重复id: ${uniqueId}，生成新id`)
            uniqueId = `item_${requestPage}_${index}_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`
          }
        ...
        const uniqueNewItems = isLoadMore 
          ? newItems.filter(item => {
              ...
              if (exists) {
                console.warn(`[fetchHotSpots] 过滤重复项: id=${item.id}`)
              }
              return !exists
            })
          : newItems
        ...
        allItems.forEach(item => {
          ...
          if (!uniqueItemsMap.has(itemId)) {
            ...
            console.warn(`[fetchHotSpots] 最终去重：发现重复id: ${itemId}，已跳过`)
          }
        })
        const finalItems = Array.from(uniqueItemsMap.values())
        
        console.log(`[fetchHotSpots] 数据处理：API返回 ${items.length} 条，处理后 ${newItems.length} 条，过滤重复后 ${uniqueNewItems.length} 条，合并后 ${allItems.length} 条，最终去重后 ${finalItems.length} 条`)
        ...
        if (isPreload) {
          ...
          console.log(`[fetchHotSpots] 预加载完成，请求页: ${requestPage}，更新后页码: ${nextPage}，hasMore: ${hasMore}，items数量: ${finalItems.length}（新增 ${newItems.length} 条）`)
        } else if (isLoadMore) {
          // 正常加载更多完成
          updateData.loadingMore = false
          updateData.preloadTriggered = false // 重置预加载标记，允许下次预加载
          console.log(`[fetchHotSpots] 加载更多完成，请求页: ${requestPage}，更新后页码: ${nextPage}，hasMore: ${hasMore}，items数量: ${finalItems.length}（新增 ${newItems.length} 条，原有 ${existingItems.length} 条）`)
        } else {
          // 首次加载完成
          updateData.preloadTriggered = false // 重置预加载标记
          console.log(`[fetchHotSpots] 首次加载完成，请求页: ${requestPage}，更新后页码: ${nextPage}，hasMore: ${hasMore}，items数量: ${finalItems.length}`)
        }
      ...
      fail: (err) => {
        ...
        if (isLoadMore) {
          if (isPreload) {
            ...
            console.error('[fetchHotSpots] 预加载失败，已重置预加载状态')
          } else {
          this.setData({ loadingMore: false })
          wx.showToast({
            title: '加载失败，请重试',
